        ...
        try (FileWriter infoFile = new FileWriter(infoFileDir + "/info" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss")) + ".txt")){
            ...
            if (dir.listFiles() != null) {
                for (File file : dir.listFiles()) {
                    ...
                    // FileSystems.getDefault().getPath(file.getAbsolutePath()) ~ Paths.get(file.getAbsolutePath())
                    infoFile.write("Created: " + Files.readAttributes(Paths.get(file.getAbsolutePath()), BasicFileAttributes.class).creationTime().toString() + "\n\n");
                }
            } else {
